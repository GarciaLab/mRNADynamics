...
if ~NoAP
    ...
    elseif strcmp(FileMode,'LSM')|strcmp(FileMode,'LIFExport')
        ...
        for i=HisChannel:NChannels:size(ImageTemp{1},1)
                MaxTemp(:,:,i)=ImageTemp{1}{i,1};
        end
        if InvertHis
            SurfImage=MaxTemp(:,:,HisChannel-1+round(NSlices/2)*NChannels-1);
        else
...
%         SurfRows=str2num(MetaFullEmbryo.getPixelsSizeY(0));
    ...
    if HistoneChannel
        ...
        D=dir([PreProcPath,filesep,Prefix,filesep,Prefix,'*_z*.tif']);
        if ~isempty(D)
            ZoomImage=imread([PreProcPath,filesep,Prefix,filesep,D(end).name],ChannelToLoad);
        else
