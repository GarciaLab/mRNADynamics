function AnalyzeLiveDataTwoColors
...
analyzeShawnLibrary('fad',@(x)tagged(x,'id',[Prefix,'_']),'params_mRNADynamics',[inf,inf])
cd([MS2CodePath])
...
cd([FISHPath,filesep,'Analysis'])
analyzeShawnLibrary('fad',@(x)tagged(x,'id',[Prefix,'_']),'params_mRNADynamics',...
    [ThresholdGFP,ThresholdRFP])
cd([MS2CodePath])


%% Track the nuclei and check their segmentation

%Use the red channel in order to generate a fake nuclear image
GenerateFakeNuclei(Prefix)
