function CheckParticleTrackingV4(varargin)
...
if isempty(varargin)
    DataFolder=uigetdir(DropboxFolder,'Select data set to analyze');
else
...
%                 EllipseHandleBlue=[EllipseHandleBlue,ellipse(Ellipses{CurrentFrame}(schnitzCellNo,3),...
%                     Ellipses{CurrentFrame}(schnitzCellNo,4),...
%                     Ellipses{CurrentFrame}(schnitzCellNo,5),...
%                     Ellipses{CurrentFrame}(schnitzCellNo,1)+1,...
...
%             ZProfileRaw);
    ...
    if (CurrentParticle~=PreviousParticle)|~exist('Amp')
        ...
        [Frames,Amp]=PlotParticleTraceV2(CurrentParticle,Particles,fad,FISHPath,FilePrefix);
    end
            ...
            if ~isempty(ClickedPosition)
                ...
                for i=1:length(schnitzcells)
                    if sum((schnitzcells(i).frames-1)==CurrentFrame)
                        IndexToSearch=find((schnitzcells(i).frames-1)==CurrentFrame);
                        try 
                ...
                if SchnitzFound==0
                    ...
                    schnitzcells(Nschnitz+1).frames=CurrentFrame+1;
        ...
        if isempty(NewNuclei)
            ...
            for i=1:NClicks
                ...
                for j=1:length(schnitzcells)
                    if sum(schnitzcells(j).frames-1==CurrentFrame)
                        SchnitzSuspect=[SchnitzSuspect,j];
                        if (~isempty(schnitzcells(j).cenx(find((schnitzcells(j).frames-1)==CurrentFrame))))&...
                                (~isempty(schnitzcells(j).ceny(find((schnitzcells(j).frames-1)==CurrentFrame))))
                            xPosSuspect=[xPosSuspect,...
                                schnitzcells(j).cenx(find((schnitzcells(j).frames-1)==CurrentFrame))];
                            yPosSuspect=[yPosSuspect,...
                                schnitzcells(j).ceny(find((schnitzcells(j).frames-1)==CurrentFrame))];
