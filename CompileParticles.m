...
if HistoneChannel&strcmp(ExperimentAxis,'AP')
    ...
    for i=1:length(CompiledParticles)
        if ~isempty(Particles(i).Nucleus)
            CompiledParticleNuclei{i}=...
                schnitzcells(CompiledParticles(i).Nucleus).cellno(ismember(schnitzcells(CompiledParticles(i).Nucleus).frames,...
                CompiledParticles(i).Frame));
            CompiledParticleFrames{i}=...
                schnitzcells(CompiledParticles(i).Nucleus).frames(ismember(schnitzcells(CompiledParticles(i).Nucleus).frames,...
                CompiledParticles(i).Frame));
        else
            CompiledParticleNuclei{i}=[];
            CompiledParticleFrames{i}=[];
    ...
    MeanVectorAllAP=nan(length(Ellipses),length(APbinID));
    SEVectorAllAP=nan(length(Ellipses),length(APbinID));
    ...
    for i=1:length(Ellipses)
        ...
        for j=1:length(CompiledParticlesToCheck)
            if EllipseFilter(CompiledParticleNuclei{CompiledParticlesToCheck(j)}(CurrentCompiledParticlesIndex{CompiledParticlesToCheck(j)}))&...
                    (CompiledParticles(CompiledParticlesToCheck(j)).Approved==1 | CompiledParticles(CompiledParticlesToCheck(j)).Approved==2)
                ...
                FluorescenceCompiledParticles=[FluorescenceCompiledParticles,...
                    CompiledParticles(CompiledParticlesToCheck(j)).Fluo(...
                ...
                ErrorFluorescenceCompiledParticles=[ErrorFluorescenceCompiledParticles,...
                    CompiledParticles(CompiledParticlesToCheck(j)).FluoError];
            end
        end
        ...
        for j=1:length(APbinID)
            if ~isnan(APbinArea(j))
                ...
                MeanVectorAllAP(i,j)=sum(FluorescenceCompiledParticles(APFilterTemp))/NEllipsesAP(i,j);
                SEVectorAllAP(i,j)=sqrt(sum((ErrorFluorescenceCompiledParticles(APFilterTemp)/NEllipsesAP(i,j)).^2));
            end
        end
    end
