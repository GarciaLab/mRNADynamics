...
while (cc~=13)
    ...
    if FitResults{j}(i,CurrentNC-11).Approved==-1
        set(gcf,'Color','r')
    elseif FitResults{j}(i,CurrentNC-11).Approved==1
        set(gcf,'Color','g')
    else
...
%             FrameFilter=NData>=MinParticles;
            ...
            %fit
            if isempty(FitResults{j}(i,CurrentNC-11).FitFrameRange)
                ...
                FitResults{j}(i,CurrentNC-11).FitFrameRange=FitFrameRange;
            else
                FitFrameRange=FitResults{j}(i,CurrentNC-11).FitFrameRange;
            end
...
%             NDataForFit=NData;
            ...
            if CurrentNC~=14
                %Do the fit
                x0=[FitResults{j}(i,CurrentNC-11).TimeStart0,FitResults{j}(i,CurrentNC-11).TimeEnd0,FitResults{j}(i,CurrentNC-11).Rate0];
                ...
                if ~isempty(TimeData(NanFilter))
                    ...
                    FitResults{j}(i,CurrentNC-11).TimeStart=xFit(1);
                    FitResults{j}(i,CurrentNC-11).TimeEnd=xFit(2);
                    FitResults{j}(i,CurrentNC-11).RateFit=xFit(3);

                    %Estimate an error bar out of the confidence intervals
                    FitResults{j}(i,CurrentNC-11).CI=nlparci(xFit,residual,'jacobian',jacobian);

                    FitResults{j}(i,CurrentNC-11).SDTimeStart=(FitResults{j}(i,CurrentNC-11).CI(1,2)-FitResults{j}(i,CurrentNC-11).CI(1,1))/2;
                    FitResults{j}(i,CurrentNC-11).SDTimeEnd=(FitResults{j}(i,CurrentNC-11).CI(2,2)-FitResults{j}(i,CurrentNC-11).CI(2,1))/2;
                    FitResults{j}(i,CurrentNC-11).SDRateFit=(FitResults{j}(i,CurrentNC-11).CI(3,2)-FitResults{j}(i,CurrentNC-11).CI(3,1))/2;
...
%                     Plot the data that was actually used for the fit
                    ...
                    legend(['tON=',num2str(FitResults{j}(i,CurrentNC-11).TimeStart),' \pm ',num2str(FitResults{j}(i,CurrentNC-11).SDTimeStart)],...
                        ['tOFF=',num2str(FitResults{j}(i,CurrentNC-11).TimeEnd),' \pm ',num2str(FitResults{j}(i,CurrentNC-11).SDTimeEnd)],...
                        ['Rate=',num2str(FitResults{j}(i,CurrentNC-11).RateFit),' \pm ',num2str(FitResults{j}(i,CurrentNC-11).SDRateFit)],...
                        'Location','SouthOutside')
                end
            elseif CurrentNC==14
                
                %Do the fit
                x0=[FitResults{j}(i,CurrentNC-11).TimeStart0,FitResults{j}(i,CurrentNC-11).Rate0];
                ...
                if ~isempty(TimeData(NanFilter))
                    ...
                    FitResults{j}(i,CurrentNC-11).TimeStart=xFit(1);
                    FitResults{j}(i,CurrentNC-11).RateFit=xFit(2);

                    %Estimate an error bar out of the confidence intervals
                    FitResults{j}(i,CurrentNC-11).CI=nlparci(xFit,residual,'jacobian',jacobian);

                    FitResults{j}(i,CurrentNC-11).SDTimeStart=(FitResults{j}(i,CurrentNC-11).CI(1,2)-FitResults{j}(i,CurrentNC-11).CI(1,1))/2;
                    FitResults{j}(i,CurrentNC-11).SDRateFit=(FitResults{j}(i,CurrentNC-11).CI(2,2)-FitResults{j}(i,CurrentNC-11).CI(2,1))/2;
    ...
    title(['Data Set = ',num2str(j),', ','Particle = ',num2str(i),'/',num2str(length(CP{j})),', ',num2str(data(j).APbinID(currentAPBin)),' AP, TimeStart0=',num2str(FitResults{j}(i,CurrentNC-11).TimeStart0),...
        ', TimeEnd0=',num2str(FitResults{j}(i,CurrentNC-11).TimeEnd0),', Rate=',num2str(FitResults{j}(i,CurrentNC-11).Rate0),...
...
%             data(j).ElapsedTime(eval(['nc',num2str(nc)]))])
    ...
    elseif (ct~=0)&(cc=='q')
        if FitResults{j}(i,CurrentNC-11).Approved==0
            FitResults{j}(i,CurrentNC-11).Approved=1;
        elseif FitResults{j}(i,CurrentNC-11).Approved==1
            FitResults{j}(i,CurrentNC-11).Approved=0;
        end
    ...
    elseif (ct~=0)&(cc=='k')&(length(FitResults{j}(i,CurrentNC-11).FitFrameRange)>2)
        FitResults{j}(i,CurrentNC-11).FitFrameRange=FitResults{j}(i,CurrentNC-11).FitFrameRange(1:end-1);
    elseif (ct~=0)&(cc=='l')
        if ~isempty(find(~ismember(FrameWindow,FitResults{j}(i,CurrentNC-11).FitFrameRange)))
            FilteredFramesTemp=FrameWindow;
            FitResults{j}(i,CurrentNC-11).FitFrameRange(end+1)=...
                FilteredFramesTemp(min(find(~ismember(FilteredFramesTemp,FitResults{j}(i,CurrentNC-11).FitFrameRange))));
        end
    %Move left range of fit
    elseif (ct~=0)&(cc=='j')&(length(FitResults{j}(i,CurrentNC-11).FitFrameRange)>2)
        FitResults{j}(i,CurrentNC-11).FitFrameRange=FitResults{j}(i,CurrentNC-11).FitFrameRange(2:end);
    elseif (ct~=0)&(cc=='h')
        if ~isempty(find(~ismember(FrameWindow,FitResults{j}(i,CurrentNC-11).FitFrameRange)))
            FilteredFramesTemp=FrameWindow;
            FitResults{j}(i,CurrentNC-11).FitFrameRange=...
                [FilteredFramesTemp(max(find(~ismember(FilteredFramesTemp,FitResults{j}(i,CurrentNC-11).FitFrameRange)))),...
                FitResults{j}(i,CurrentNC-11).FitFrameRange];
        end
    %Reset frame fit range
     elseif (ct~=0)&(cc=='r')   
        FitResults{j}(i,CurrentNC-11).FitFrameRange=FrameWindow;

        
        
    %Change the initial parameters
    %TimeStart
    elseif (ct~=0)&(cc=='a')&((CurrentNC==14)|(CurrentNC~=14&FitResults{j}(i,CurrentNC-11).TimeStart0<FitResults{j}(i,CurrentNC-11).TimeEnd0))
        FitResults{j}(i,CurrentNC-11).TimeStart0=FitResults{j}(i,CurrentNC-11).TimeStart0+1;
    elseif (ct~=0)&(cc=='z')&(FitResults{j}(i,CurrentNC-11).TimeStart0>1)
        FitResults{j}(i,CurrentNC-11).TimeStart0=FitResults{j}(i,CurrentNC-11).TimeStart0-1;
    %TimeEnd
    elseif (ct~=0)&(cc=='s')&(FitResults{j}(i,CurrentNC-11).TimeEnd0<data(j).ElapsedTime(FrameWindow(end))-data(j).ElapsedTime(FrameWindow(1)))
        FitResults{j}(i,CurrentNC-11).TimeEnd0=FitResults{j}(i,CurrentNC-11).TimeEnd0+1;
    elseif (ct~=0)&(cc=='x')&(FitResults{j}(i,CurrentNC-11).TimeEnd0>FitResults{j}(i,CurrentNC-11).TimeStart0)
        FitResults{j}(i,CurrentNC-11).TimeEnd0=FitResults{j}(i,CurrentNC-11).TimeEnd0-1;
    %Rate, fine
    elseif (ct~=0)&(cc=='c')&(FitResults{j}(i,CurrentNC-11).Rate0>100)
        FitResults{j}(i,CurrentNC-11).Rate0=FitResults{j}(i,CurrentNC-11).Rate0-100;
    elseif (ct~=0)&(cc=='d')
        FitResults{j}(i,CurrentNC-11).Rate0=FitResults{j}(i,CurrentNC-11).Rate0+100;    
    %Rate, coarse
    elseif (ct~=0)&(cc=='C')&(FitResults{j}(i,CurrentNC-11).Rate0>100)
        FitResults{j}(i,CurrentNC-11).Rate0=FitResults{j}(i,CurrentNC-11).Rate0-500;
    elseif (ct~=0)&(cc=='D')
        FitResults{j}(i,CurrentNC-11).Rate0=FitResults{j}(i,CurrentNC-11).Rate0+500; 
    ...
    %Save
    elseif (ct~=0)&(cc=='v')
        save([DropboxFolder,filesep,Prefix,filesep,'FitResults.mat'],...
        'FitResults')
