function [ xy, mapping, nuclei ] = trackMitosis( names, firstFrameNumber, lastFrameNumber, shifts, diameter, embryoMask, xy, manualMapping, nuclei, varargin )
...
maxShiftCorrection = getDefaultParameters('max Shift Correction', 'trackToTheNextFrame')*diameter/space_resolution;
nFrames = (lastFrameNumber-firstFrameNumber+1);
edgeClearance = getDefaultParameters('edge clearance')*diameter/space_resolution;
...
if ~exist('xy','var') || isempty(xy)
    for j = 1:nFrames
        ...
        tmp = findNuclei(names,firstFrameNumber-1+j,diameter,embryoMask);
