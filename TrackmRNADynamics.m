...
if strcmp(ExperimentType,'1spot')||strcmp(ExperimentType,'2spot')||strcmp(ExperimentType,'inputoutput')
    ...
    elseif strcmp(ExperimentType,'inputoutput')
        ...
        if length(SpotsChannel)>1
            error('only one output channel currently supported')
        end
   end
    ...
    for i=1:length(Spots) %iterate over all frames
        ...
        elseif length(D)==2
            Image=imread([PreProcPath,filesep,Prefix,filesep,FilePrefix,iIndex(CurrentFrame,3),'_z',iIndex(CurrentZ,2),'_ch',iIndex(SpotsChannel,2),'.tif']);
        else
        ...
        if ~UseHistone
            ...
            if ~isempty(x)
                ...
                if isempty(Particles)
                    ...
                    [PreviousSpotsX,PreviousSpotsY]=SpotsXYZ(Spots(CurrentFrame-1));
                    for j=1:length(Particles)
                        if Particles(j).Frame(end)==(CurrentFrame-1)
                            PreviousFrameParticles=[PreviousFrameParticles,j];
                            PreviousFrameIndex=Particles(j).Index(end);
                    ...
                    for j=1:length(NewParticles)
                        ...
                        Particles(TotalParticles+1).Frame=CurrentFrame;
