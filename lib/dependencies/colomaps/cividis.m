...
%   CIVIDIS(m) returns an M-by-3 matrix containing the cividis colormap, a
%   better, alternative colormap to the MATLAB defaults, which is designed 
%   to be:
%       1) COLORFUL, spanning as wide a palette (perceptual range) as 
%           possible so as to make differences easy to see;
%       2) PERCEPTUALLY UNIFORM, meaning that values close to each other 
%           have similar-appearing colors and values far away from each 
%           other have more different-appearing colors, consistently across
%           the range of values, such that delta(color) = delta(data);
%       3) ROBUST TO COLORBLINDNESS, so that the above properties hold true
%           for people with common forms of colorblindness, as well as in 
%           grey scale printing; and 
%       4) PRETTY!
%   The colors begin with dark blue (lowest values), range through shades 
%   of blue and blue-yellow, and end in yellow (highest values). 
%   CIVIDIS, by itself, is the same length as the current figure's 
%   colormap. If no figure exists, MATLAB uses the length of the default 
%   colormap.
%
%   Other perceptually uniform colormaps include VIRIDIS, PLASMA,
%   MAGMA, and INFERNO.
%
