function FitSingleParticleMCMC(varargin)

%Last updated on: 8/10/19 by Jonathan Liu
...
%using Markov Chain Monte Carlo, for each single particle
...
%file for each dataset, called SingleParticleFitsMCMC.mat, to be used for later
%analysis.
...
%structure param_log, inside the SingleParticleFitsMCMC.mat file.
...
if iscell(data.CompiledParticles)
    ...
    CompiledParticles = data.CompiledParticles;
end
...
%bins = 0:APResolution:1; %nuc resolution
%N_bins = length(bins); %Total number of bins
N_particles = length(CompiledParticles);
N_cycles = 2; %Number of nuclear cycles

%% MCMC analysis loop through nuclear cycles and particle number
...
MCMCchain_temp(N_particles) = struct();
MCMCresults_temp(N_particles) = struct();
MCMCplot_temp(N_particles) = struct();

for nuc = 1:N_particles
    MCMCchain_temp(nuc).ton_chain = [];
    MCMCchain_temp(nuc).MS2_basal_chain = [];
    MCMCchain_temp(nuc).dwelltime_chain = [];
    MCMCchain_temp(nuc).R0_chain = [];
    MCMCchain_temp(nuc).dR_chain = [];
    MCMCchain_temp(nuc).s2_chain = [];

    MCMCresults_temp(nuc).mean_ton = nan;
    MCMCresults_temp(nuc).sigma_ton = nan;
    MCMCresults_temp(nuc).mean_MS2_basal = nan;
    MCMCresults_temp(nuc).sigma_MS2_basal = nan;
    MCMCresults_temp(nuc).mean_dwelltime = nan;
    MCMCresults_temp(nuc).sigma_dwelltime = nan;
    MCMCresults_temp(nuc).mean_R0 = nan;
    MCMCresults_temp(nuc).sigma_R0 = nan;
    MCMCresults_temp(nuc).mean_dR = nan;
    MCMCresults_temp(nuc).sigma_dR = nan;
    MCMCresults_temp(nuc).mean_sigma = nan;
    MCMCresults_temp(nuc).sigma_sigma = nan;
    MCMCresults_temp(nuc).MeanAP = CompiledParticles(nuc).MeanAP;
    MCMCresults_temp(nuc).ApprovedFits = 0; %Set approval status to uncurated (0).
    MCMCresults_temp(nuc).nc = nc_string{nc};

    MCMCplot_temp(nuc).t_plot = [];
    MCMCplot_temp(nuc).MS2_plot = [];
    MCMCplot_temp(nuc).t_interp = [];
    MCMCplot_temp(nuc).MS2_interp = [];
    MCMCplot_temp(nuc).simMS2 = [];
    MCMCplot_temp(nuc).firstnc13time = [];
    MCMCplot_temp(nuc).lastnc13time = [];
    MCMCplot_temp(nuc).firstnc14time = [];
    MCMCplot_temp(nuc).lastnc14time = [];
    MCMCplot_temp(nuc).nc = nc_string{nc};
end

parfor nuc = 1:N_particles
%Extract particle fluorescence information

%MS2 fluorescence data
Frame = CompiledParticles(nuc).Frame;
MS2_raw = CompiledParticles(nuc).Fluo ./ scalefac;
FramesInNC = sum(and(Frame > nc_array(nc), Frame < nc_array(nc+1)));

%Skip this nuc bin if there are fewer than 10 datapoints in the nuclear
%cycle
if FramesInNC < 10
...
%Save fluorescence as a time series, rather than indexed by frame
MS2 = nan(size(t));
MS2(Frame) = MS2_raw;
...
%% Save data
MCMCchain_temp(nuc).ton_chain = ton_chain;
MCMCchain_temp(nuc).MS2_basal_chain = MS2_basal_chain;
MCMCchain_temp(nuc).dwelltime_chain = dwelltime_chain;
MCMCchain_temp(nuc).R0_chain = R0_chain;
MCMCchain_temp(nuc).dR_chain = dR_chain;
MCMCchain_temp(nuc).s2chain = s2chain;

MCMCresults_temp(nuc).mean_ton = mean_ton;
MCMCresults_temp(nuc).sigma_ton = sigma_ton;
MCMCresults_temp(nuc).mean_dwelltime = mean_dwelltime;
MCMCresults_temp(nuc).sigma_dwelltime = sigma_dwelltime;
MCMCresults_temp(nuc).mean_MS2_basal = mean_MS2_basal;
MCMCresults_temp(nuc).sigma_MS2_basal = sigma_MS2_basal;
MCMCresults_temp(nuc).mean_R0 = mean_R0;
MCMCresults_temp(nuc).sigma_R0 = sigma_R0;
MCMCresults_temp(nuc).mean_dR = mean_dR;
MCMCresults_temp(nuc).sigma_dR = sigma_dR;
MCMCresults_temp(nuc).mean_sigma = mean_sigma;
MCMCresults_temp(nuc).sigma_sigma = sigma_sigma;

MCMCplot_temp(nuc).t_plot = t_plot;
MCMCplot_temp(nuc).MS2_plot = MS2_plot;
MCMCplot_temp(nuc).t_interp = t_interp;
MCMCplot_temp(nuc).MS2_interp = MS2_interp;
MCMCplot_temp(nuc).simMS2 = simMS2;
MCMCplot_temp(nuc).firstnc13time = firstnc13time;
MCMCplot_temp(nuc).lastnc13time = lastnc13time;
MCMCplot_temp(nuc).firstnc14time = firstnc14time;
MCMCplot_temp(nuc).lastnc14time = lastnc14time;
MCMCplot_temp(nuc).nc = nc_string{nc};
end
...
filename = 'SingleParticleFitsMCMC';
save([loc,filename,'.mat'],'MCMCresults','MCMCplot','Prefix','param_log');

filename = 'SingleParticleFitsMCMC_RawChains';
