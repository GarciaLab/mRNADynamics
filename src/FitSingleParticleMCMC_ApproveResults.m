function FitSingleParticleMCMC_ApproveResults(varargin)
%Last updated: 8/11/19 by Jonathan Liu

%Analyzes saved MCMC results from FitSingleParticleMCMC. The user has the option of
...
%Load MCMC results                                
m = load([DropboxFolder,filesep,Prefix,'\SingleParticleFitsMCMC.mat']);
N_particles = length(m.MCMCplot{1}); %Number of particles in this dataset
...
if RawChains
    chains = load([DropboxFolder,filesep,Prefix,'\SingleParticleFitsMCMC_RawChains.mat']);
...
Function = {'Approve this fit';'Reject this fit';'Previous particle';...
    'Next particle';'Jump to a specific particle';'Previous nuclear cycle';...
...
MeanAP = MCMCresults{nc}(i).MeanAP;
...
title({['MCMC fit: Particle ',num2str(i),', AP Position ',num2str(MeanAP), ', Nuclear cycle ',num2str(nuclearcycle)]...
...
save([DropboxFolder,filesep,Prefix,'\SingleParticleFitsMCMC.mat'],'MCMCresults','MCMCplot','Prefix');
