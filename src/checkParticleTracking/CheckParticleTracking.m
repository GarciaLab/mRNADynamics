...
[sortByFrames, sortByLength, ForCompileAll, SpeedMode, ~, ...
    ...
    UseTwinTraces, PlotInputChannel, UseCompiledParticles] = determineCheckParticleTrackingOptions(varargin{:});



liveExperiment = LiveExperiment(Prefix, preMovie);

% Added by G. Martini 2/15/21
if isempty(UseTwinTraces)
...
if isempty(preStructs)
    ...
    if strcmpi(liveExperiment.experimentType, '2spot') % Added by G. Martini 2/13/21
        if iscell(Particles)
            for particleChannelIndex = 1:length(Particles)
                if ~isfield(Particles{particleChannelIndex}, 'Schnitz')
                    for particleIndex = 1:length(Particles{particleChannelIndex})
                        if ~isempty(Particles{particleChannelIndex}(particleIndex).Nucleus)
                            Particles{particleChannelIndex}(particleIndex).Schnitz = Particles{particleChannelIndex}(particleIndex).Nucleus;
                        else
                            Particles{particleChannelIndex}(particleIndex).Schnitz = NaN;
                        end
                    end
                end      
            end
        else
            if ~isfield(Particles, 'Schnitz')
                for particleIndex = 1:length(Particles)
                    if ~isempty(Particles(particleIndex).Nucleus)
                        Particles(particleIndex).Schnitz = Particles(particleIndex).Nucleus;
                    else
                        Particles(particleIndex).Schnitz = NaN;
                    end
                end
            end
        end
...
if ~UseCompiledParticles
    ...
    [CompiledParticles] = sortParticles(sortByFrames, sortByLength, numSpotChannels, CompiledParticles);
end
...
if ~UseCompiledParticles
    cptState = CPTState(liveExperiment, Spots, Particles, SpotFilter, schnitzcells, Ellipses,...
        FrameInfo, UseHistoneOverlay, nWorkers, plot3DGauss, projectionMode, UseTwinTraces, PlotInputChannel); %work in progress, 2019-12, JP.
else
    cptState = CPTState(liveExperiment, Spots, CompiledParticles, SpotFilter, schnitzcells, Ellipses,...
...
if ncRange
    ...
    particlesInRange = particlesWithinFrames(Prefix, firstNCFrame, lastNCFrame, UseCompiledParticles, cptState.CurrentChannel);
...
%Define user interface - GM STOPPED HERE 12/10
[Overlay, overlayAxes, snippetFigAxes,...
    ...
    cptState.plot3DGauss, ExperimentType, multiView, xSize, ySize, cptState.PlotInputChannel);
...
twinParticleKeyInput = TwinParticleEventHandler(cptState);
...
plotTraceSettings = PlotTraceSettings(cptState.UseTwinTraces,  cptState.PlotInputChannel, cptState.UseCompiledParticles);

% cptState.CurrentParticle = 286;
% cptState.CurrentFrame = 60;
% cptState.TwinParticle = 48;
%% Main loop - start
while (currentCharacter ~= 'x')
    ...
    if cptState.UseTwinTraces
        plotTwinPositions(zProfileFigAxes, zTraceAxes, cptState)
    end
...
%%
    ...
    twinParticleKeyInput(currentCharacter);
