...
% :/" :Move to the next/previous unapproved frame within a particle
...
[sortByFrames, sortByLength, sortByFlags, ForCompileAll, SpeedMode, ~, ...
...
liveExperiment = LiveExperiment(Prefix, 1);
...
[Particles] = sortParticles(sortByFrames, sortByLength, sortByFlags, numSpotChannels, Particles);
...
while (currentCharacter ~= 'x')        
    ...
    qcAxes = plotQCDiagnostics(qcAxes, cptState);
