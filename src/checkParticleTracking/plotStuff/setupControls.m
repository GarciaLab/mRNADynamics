function [controls_fig, frame_num, z_num, particle_num, ...
    add_spot, smart_add_spot, delete_spot, fit_spot,...
    averagingLength, approve_fit] = setupControls(Overlay)
...
fit_spot = uibutton('Parent', controls,...
    'Position', [0.05 * dim(1), 0.2 * dim(2) + offset, 0.4 * dim(1), 0.08 * dim(2)], ...
    'Text', 'Manual Fit', 'FontSize', 10);
uilabel('Parent', controls,'Text', 'Averaging window: ', 'Position', ...
    [0.05 * dim(1), 0.1 * dim(2) + offset, 0.35 * dim(1), 0.08 * dim(2)], 'FontSize', 10);
averagingLength = uieditfield('Parent', controls, 'Value', '1', 'Position', ...
    [.4 * dim(1), 0.1 * dim(2) + offset, .4 * dim(1), 0.08 * dim(2)], ...
    'FontSize', 12);
uilabel('Parent', controls,'Text', 'Averaging window: ', 'Position', ...
    [0.05 * dim(1), 0.1 * dim(2) + offset, 0.35 * dim(1), 0.08 * dim(2)], 'FontSize', 10);
approve_fit = uibutton('Parent', controls,...
    'Position', [0.45 * dim(1), 0.2 * dim(2) + offset, 0.4 * dim(1), 0.08 * dim(2)], ...
    'Text', 'Approve Fit', 'FontSize', 10);
end
