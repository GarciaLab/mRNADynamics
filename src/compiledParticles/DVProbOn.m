...
for ChN=1:NChannels
    ...
    if ~isempty(Particles{ChN})
        ...
        for i=1:length(Particles{ChN})
            s = Particles{ChN}(i).Nucleus;
            if ~isempty(s) & s~=0
                ParticleNuclei{i}=...
                    schnitzcells(s).cellno(ismember(schnitzcells(s).frames,...
                    Particles{ChN}(i).Frame));
                ParticleFrames{i}=...
                    schnitzcells(s).frames(ismember(schnitzcells(s).frames,...
                    Particles{ChN}(i).Frame));
            else
                ParticleNuclei{i}=[];
                ParticleFrames{i}=[];
        ...
        for i=1:length(CompiledParticles{ChN})
            s = CompiledParticles{ChN}(i).Nucleus;
            if ~isempty(s)
        ...
        for i=1:length(Ellipses)
            ...
            for j=1:length(ParticlesToCheck)
                if EllipseFilter(ParticleNuclei{ParticlesToCheck(j)}(CurrentParticlesIndex{ParticlesToCheck(j)}))&...
                    ...
                    FilteredParticlesPosDV=[FilteredParticlesPosDV,...
            ...
            EllipsesFilteredPosDV{i}=EllipsePos_DV{i}(EllipseFilter);
            for j=1:length(EllipsesFilteredPosDV{i})
                NEllipsesDV(i,max(find(DVbinID<=EllipsesFilteredPosDV{i}(j))))=...
                    NEllipsesDV(i,max(find(DVbinID<=EllipsesFilteredPosDV{i}(j))))+1;
            end
            
            
            %Count the number of filtered particles per DV bin.
            for j=1:length(FilteredParticlesPosDV)
                NParticlesEllipsesDV{ChN}(i,max(find(DVbinID<=FilteredParticlesPosDV(j))))=...
                    NParticlesEllipsesDV{ChN}(i,max(find(DVbinID<=FilteredParticlesPosDV(j))))+1;
            end
