function particleFiltered = filterParticleTraces(particle, trackingOptions)

    % remove problematic frames
    FrameFilter = particle.FrameApproved;
    particleFiltered = struct;
    ...
    if isfield(particle, 'xPos3D')
        ...
        if isfield(particle, 'zPos')
            particleFiltered.zPos3D = particle.zPoseD(FrameFilter);
        end
    end
    ...
    particleFiltered.nucleusProbability = particle.nucleusProbability ;
...
%     trackingStruct.MeasurementVec(:,end+1) = FluoVec./trackingOptions.kalmanOptions.fluoFactor;
    ...
    if trackingOptions.has3DInfo
        particleFiltered = add3DTrackingInfo(particleFiltered, Spots, trackingOptions, trackingOptions.kalmanOptions);
    end
