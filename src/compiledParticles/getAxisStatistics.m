...
MeanVectorAll = {}; MeanVectorAP = {}; SDVectorAP = {};
...
for ChN=1:NChannels
    if ~isempty(CompiledParticles{ChN})
        ...
        [AllTracesVector{ChN},AllTracesAP{ChN},AllTracesDV{ChN}]=...
            createAllTracesVector(FrameInfo,CompiledParticles{ChN},'NoAP');
        ...
        if ( strcmpi(ExperimentAxis,'AP') || strcmpi(ExperimentAxis,'DV') )...
            ...
            if ROI
                ...
                k=1;
                for i=MinAPIndex:MaxAPIndex
                    [MeanVectorAPTemp_nonROI,SDVectorAPTemp_nonROI,NParticlesAPTemp_nonROI]...
                        ...
                        =AverageTraces(...
                        FrameInfo, CompiledParticles_nonROI{ChN}(APFilter_nonROI{ChN}(:,i)));
        ...
        if strcmpi(ExperimentAxis,'DV')
            ...
            % This is the case which we don't use ROI option
            %Get the corresponding mean information
            k=1;
            for i=MinDVIndex:MaxDVIndex
                [MeanVectorDVTemp,SDVectorDVTemp,NParticlesDVTemp]=AverageTraces(FrameInfo,...
        ...
        for i=1:length(NewCyclePos)
            if i==1
                ...
                peakFrame{ChN}=[peakFrame{ChN},MaxIndex];
            elseif i<=length(NewCyclePos)
                [~,MaxIndex]=max(MeanVectorAll{ChN}(NewCyclePos(i-1):NewCyclePos(i)));
                peakFrame{ChN}=[peakFrame{ChN},NewCyclePos(i-1)+MaxIndex-1];
            end
        end
        ...
        if ~isempty(NewCyclePos)        %Why is this empty sometimes?
            ...
            %data
            peakFrame{ChN}=[peakFrame{ChN},NewCyclePos(i)+MaxIndex-1];
        end
    end
end
end
