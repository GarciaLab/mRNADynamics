    ...
    for frame = 1:nFrames %frames
        ...
        if ~displayFigures
            ...
            parfor spot = 1:nSpotsPerFrame
                ...
                SpotsFrame(spot).fits3D = fits;
                SpotsFrame(spot).gauss3DIntensity = intensity;
            end
        else
            for spot = 1:nSpotsPerFrame
                ...
                %         snips3D = [snips3D, currentSnippet3D]; %#ok<*AGROW>
                initial_params = [max(max(max(snip3D))), NaN,NaN, snipDepth + 1, width,offsetGuess];
                                    [fits, intensity] = fitGaussian3D(snip3D, initial_params, 'displayFigures');
