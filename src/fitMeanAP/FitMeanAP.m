...
% FitMeanAP(varargin)
% varargin{1} : Prefix
% varargin{2} : Type of fitting ('Symmetric', 'Asymmetric', 'Linear')
% varargin{3} : Dropbox folder if needs to be directd to a specific folder.
...
%OUTPUT:MeanFits_(Option).mat
% where Option could be 'Symmetric', 'Asymmetric', or 'Linear', depending
% on which fitting method was chosen in the beginning.
...
% Initialize the Fitting Type variables
...
if ~isempty(varargin)
    Prefix=varargin{1};
    % Go through the addtional arguments
...
if ~isempty(varargin)
    if length(varargin) == 1
        error('Fitting method should be plugged in.')
