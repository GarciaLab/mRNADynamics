function FitMeanAPAsymmetric_competent(varargin)
...
while (cc~=13)
    ...
    if APDivision(CurrentNC,i)
        ...
        if (sum(NParticlesAP(FrameWindow,i)>=MinParticles)>=MinTimePoints)
            ...
            % Calculate the mean over "competent" nuclei
            ...
            elseif CurrentNC==14
                ...
                if ~isempty(TimeData(NanFilter))
                    ...
                    PlotHandle=errorbar(ElapsedTime(FrameWindow)-ElapsedTime(FrameWindow(1)),...
                        ...
                        SDVectorAP(FrameWindow,i)./...
                        sqrt(max(NParticlesAP(FrameWindow,i))),'.-k');
                    
%                     PlotHandle=errorbar(ElapsedTime(FrameWindow)-ElapsedTime(FrameWindow(1)),...
%                         MeanVectorAP(FrameWindow,i),...
%                         SDVectorAP(FrameWindow,i),'.-b');
                    hold on
                    %Plot the data that could be used for the fit
                    PlotHandle(end+1)=plot(ElapsedTime(FrameWindow(FrameFilter))-ElapsedTime(FrameWindow(1)),...
                        FluoData,'or');
...
% Made this compatible with other fitting methods in the decay regime.
% This could be all separated/factored out for a specific subscript.
if exist('NC14DecayRegimeFitResults')&& exist('NC14DecayRegimeNumericalResults')
    save([DropboxFolder,filesep,Prefix,filesep,'MeanFitsAsymmetric.mat'],...
    'FitResults','NC14DecayRegimeFitResults','NC14DecayRegimeNumericalResults')
elseif exist('NC14DecayRegimeFitResults')
        save([DropboxFolder,filesep,Prefix,filesep,'MeanFitsAsymmetric.mat'],...
    'FitResults','NC14DecayRegimeFitResults')
elseif exist('NC14DecayRegimeNumericalResults')
        save([DropboxFolder,filesep,Prefix,filesep,'MeanFitsAsymmetric.mat'],...
    'FitResults','NC14DecayRegimeNumericalResults')
else
    save([DropboxFolder,filesep,Prefix,filesep,'MeanFitsAsymmetric.mat'],...
    'FitResults')
end
