...
nStates = length(tracks(1).kalmanFilter.State);
...
for k = 1:length(tracks)
    if length(schnitzcells) < k
        ...
        schnitzcells(k).ceny(1) =  max(tracks(k).kalmanFilter.State(3), 0);
        schnitzcells(k).smaj(1) =  max(tracks(k).kalmanFilter.State(5), 0);
        schnitzcells(k).smin(1) =  max(tracks(k).kalmanFilter.State(7),0);
        schnitzcells(k).orientationAngle(1) = max(tracks(k).kalmanFilter.State(9), 0);
        ...
        schnitzcells(k).ceny(end+1) = max(tracks(k).kalmanFilter.State(3), 0);
        schnitzcells(k).smaj(end+1) = max(tracks(k).kalmanFilter.State(5), 0);
        schnitzcells(k).smin(end+1) = max(tracks(k).kalmanFilter.State(7), 0);
        schnitzcells(k).orientationAngle(end+1) = max(tracks(k).kalmanFilter.State(9), 0);
