function [falsePositives, Spots2] = findBrightestNanocageZ(num_shadows,...
...
if ~isempty(Spots)
    ...
    for frame = 1:length(Spots)
        ...
        for spotIndex = 1:nSpots
            ...
            GaussianIntensityVec = [Spots(frame).Fits(spotIndex).GaussianIntensity];
            %find slice with brightest pixel
            [~, MaxIndexCentral] = max(GaussianIntensityVec);
