...
for nSpots = 2    
    ...
    savePath = [FigurePath saveFolder filesep];
    mkdir(savePath);
    
    % define function to make noisy snip background
    makeOffsetSnip = @(params) normrnd(0,.5*params(11),size(mesh_z)) + ...
                                    params(11) + ...
    ...
    parfor n = 1:nSpotsToSimulate
        ...
        fitInfo = fit3DGaussians(validationInfo(n).simStack,liveExperiment.pixelSize_nm,liveExperiment.zStep_um*1000,[],2,[]);    

        trueInfo = fit3DGaussians(validationInfo(n).simStack,liveExperiment.pixelSize_nm,liveExperiment.zStep_um*1000,[],nSpots,trueParams);

        % calculate average for results            
        validationInfo(n).fitInfo = fitInfo;
        ...
        validationInfo(n).fitSpot1Fluo = fitInfo.GaussIntegral1;
        validationInfo(n).fitSpot2Fluo = fitInfo.GaussIntegral2;
        validationInfo(n).fitTotalFluo = fitInfo.GaussIntegralTot;


        validationInfo(n).trueTotalFluoRaw = trueInfo.GaussIntegralRaw;   

        validationInfo(n).fitTotalFluoRaw = fitInfo.GaussIntegralRaw;
        ...
        validationInfo(n).fitSpot1Pos = fitInfo.Spot1Pos;
        validationInfo(n).fitSpot2Pos = fitInfo.Spot2Pos;
        validationInfo(n).fitCenterPos = fitInfo.SpotCentroid;
    end
    validationInfoCell{nSpots} = validationInfo;
    disp('Done.')  
    ...
    saveas(centroidFig,[savePath 'position_validation.png'])
    ...
    set(gca,'FontSize',14)
    saveas(fluoIntFig,[savePath 'spot_integral_validation.png'])
    ...
    set(gca,'FontSize',14)
    saveas(posErrFig,[savePath 'position_error_plot.png'])
    ...
    saveas(fluoErrFig,[savePath 'fluo_error_plot.png'])
end
