...
function [anaphaseFrames, projectionChannels, ProjectionType, hisMat, IncludedSlices] =...
    chooseCustomProjection(Prefix, varargin)
...
nFrames = length([FrameInfo(:).Time]);
IncludedSlices = ones(nFrames,liveExperiment.zDim,'logical'); 

IncludedSlicesFile = [DropboxFolder,filesep,Prefix,filesep, 'IncludedSlices.mat'];

if exist(IncludedSlicesFile, 'file')
    load(IncludedSlicesFile, 'IncludedSlices');
end
...
median_custom_proj = cell(NChannels, ceil(sum(NFrames) / skip_factor));
max_custom_proj = cell(NChannels, ceil(sum(NFrames) / skip_factor));
sum_custom_proj = cell(NChannels, ceil(sum(NFrames) / skip_factor));
