function MeanProfiles = ArchivedCalculateMS2APProfiles(Prefix, deltaTbinWidth, MinTimePoints,UseManualApproval, verbose)
...
for ChN=1:NChannels
    ...
    schnitzFirstTranscribingFrameContainedInTrace =zeros(1, length(schnitzcells), 'logical');
    ...
    schnitzSpotStateCanBeCalled = zeros(1, length(schnitzcells), 'logical');
    for sc_idx = 1:length(schnitzcells)
        if ~isempty(schnitzcells(sc_idx).cycle)
            schnitzCycles(sc_idx) = schnitzcells(sc_idx).cycle;
            if min(schnitzcells(sc_idx).frames) >= EarliestTurnOnTimeCells(schnitzcells(sc_idx).cycle-8).
                schnitzFirstTranscribingFrameContainedInTrace(sc_idx) = true;
            end
        end
        ...
        if ~isempty(schnitzcells(sc_idx).anaphaseFrame)
            ...
            if ~isempty(schnitzcells(sc_idx).inferredAnaphaseFrame) & schnitzcells(sc_idx).anaphaseFrame > 0
