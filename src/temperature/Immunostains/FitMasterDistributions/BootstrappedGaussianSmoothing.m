...
exp_indices = [1 2 3 3 9 14];
ProfStringsCh3 = {'FitSlideRescaledDorsalAvgAPProfiles', 'FitSlideRescaledDorsalAvgAPProfiles', 'FitSlideRescaledDorsalAvgAPProfiles',...
    'FitSlideRescaledDorsalAvgAPProfiles', 'FitSlideRescaledDorsalAvgAPProfiles', 'FitSlideRescaledDorsalAvgAPProfiles'};
...
for i = 1:length(exp_indices)
    ...
    for ch_index = [3 5]
        ...
        if ch_index == 3
            ProfStr = ProfStringsCh3{i};
            ys = CompiledEmbryos.(ProfStr)(UseTF, :, ch_index);
        else
...
[CombinedSlideMeanProfiles, CombinedSlideSEs] =  CombinedBootstrappedGaussianSmoothing(AllCompiledEmbryos);
for i = 1:length(CombinedSlideMeanProfiles)
    MeanSmoothedProfiles{end+1} = CombinedSlideMeanProfiles{i};
    SmoothedProfileSEs{end+1} = CombinedSlideSEs{i};
end
%%
