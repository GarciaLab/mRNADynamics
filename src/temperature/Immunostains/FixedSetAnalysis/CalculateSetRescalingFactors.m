...
MasterFittingBins = zeros(NChannels, NumAPbins, 'logical');
MasterFittingBins(2,:) = APbins >= 0.2 &  APbins <= 0.8 ;
MasterFittingBins(3,:) = APbins >= 0.1 &  APbins <= 0.5 ;
MasterFittingBins(4,:) = APbins >= 0.5 &  APbins <= 0.9 ;
MasterFittingBins(5,:) = (APbins >= 0.2 &  APbins <= 0.5) | (APbins >= 0.7 * APbins <= 0.9) ;
%%
...
for i = 1:NumSets
    counts_windowed(:,:,:,i) = AllCompiledEmbryos{1}.WindowedProfiles.DubuisTime.Control.count;
    ControlledProfiles(:,:,:,i) = AllCompiledEmbryos{exp_idx(i)}.WindowedProfiles.DubuisTime.Control.mean;
end
FitInfo = cell(NumSets, NChannels);
ScalingFactors = NaN(NumSets,NChannels);
ScalingSEs = NaN(NumSets, NChannels);
ScalingIntercepts = NaN(NumSets,  NChannels);
ScalingInterceptSEs = NaN(NumSets, NChannels);
GoodOverlapIndices = cell(NumSets,  NChannels);

OkFitInfo = cell(NumSets, NChannels);
OkScalingFactors = NaN(NumSets,NChannels);
OkScalingSEs = NaN(NumSets, NChannels);
OkScalingIntercepts = NaN(NumSets, NChannels);
OkScalingInterceptSEs = NaN(NumSets,  NChannels);
OkOverlapIndices = cell(NumSets,  NChannels);
for i = 1:(NumSets)
    exp1 = exp_idx(i);
    for ch_index = 2:NChannels
        ...
        if sum((sum(GoodOverlapIndices{exp1, ch_index}, 2) >= 20)) > 8
            ...
            FitSet1 = CombinedMean(:,:,ch_index);
            FitSet1 = FitSet1( :,MasterFittingBins(ch_index, :));
            FitSet1 = FitSet1(GoodOverlapIndices{exp1, ch_index}(:,MasterFittingBins(ch_index, :)));
            FitSet1 = FitSet1(:);
            FitSet2 = ControlledProfiles(:,:,ch_index,i);
            FitSet2 = FitSet2(:,MasterFittingBins(ch_index, :));
            FitSet2 = FitSet2( GoodOverlapIndices{exp1, ch_index}(:,MasterFittingBins(ch_index, :)));
            FitSet2 = FitSet2(:);
            dlm = fitlm(FitSet2, FitSet1);
            FitInfo{exp1, ch_index} = dlm;
            ScalingFactors(exp1, ch_index) = dlm.Coefficients.Estimate(2);
            ScalingSEs(exp1, ch_index) = dlm.Coefficients.SE(2);
            ScalingIntercepts(exp1, ch_index) = dlm.Coefficients.Estimate(1);
            ScalingInterceptSEs(exp1, ch_index) = dlm.Coefficients.SE(1);
            
        end
        
        if sum((sum(OkOverlapIndices{exp1, ch_index}, 2) >= 20)) > 4
            FitSet1 = CombinedMean(:,:,ch_index);
            FitSet1 = FitSet1( :,MasterFittingBins(ch_index, :));
            FitSet1 = FitSet1(OkOverlapIndices{exp1, ch_index}(:,MasterFittingBins(ch_index, :)));
            FitSet1 = FitSet1(:);
            FitSet2 = ControlledProfiles(:,:,ch_index,i);
            FitSet2 = FitSet2( :,MasterFittingBins(ch_index, :));
            FitSet2 = FitSet2(OkOverlapIndices{exp1, ch_index}(:,MasterFittingBins(ch_index, :)));
            FitSet2 = FitSet2(:);
            dlm = fitlm(FitSet2, FitSet1);
            OkFitInfo{exp1, ch_index} = dlm;
            OkScalingFactors(exp1, ch_index) = dlm.Coefficients.Estimate(2);
            OkScalingSEs(exp1, ch_index) = dlm.Coefficients.SE(2);
            OkScalingIntercepts(exp1, ch_index) = dlm.Coefficients.Estimate(1);
            OkScalingInterceptSEs(exp1, ch_index) = dlm.Coefficients.SE(1);
...
for ch_index = 2:NChannels
    for i = 1:NumSets
        SetScalingFactors(i, ch_index) = ScalingFactors(i, ch_index);
        SetScalingSEs(i, ch_index) = ScalingSEs(i, ch_index);
        SetScalingIntercepts(i, ch_index) = ScalingIntercepts(i, ch_index);
        SetScalingInterceptSEs(i, ch_index) = ScalingInterceptSEs(i, ch_index);
        if isnan(SetScalingFactors(i, ch_index) )
            SetScalingFactors(i, ch_index) = OkScalingFactors(i, ch_index);
            SetScalingSEs(i, ch_index) = OkScalingSEs(i, ch_index);
            SetScalingIntercepts(i, ch_index) = OkScalingIntercepts(i,ch_index);
            SetScalingInterceptSEs(i, ch_index) = OkScalingInterceptSEs(i, ch_index);
        end
...
for exp_index = 1:length(AllSetInfo.Temperatures)
    ...
    CompiledEmbryos.MasterSetWindowedDubuisTimesControlScalingFactors = SetScalingFactors(exp_index,:);
    CompiledEmbryos.MasterSetWindowedDubuisTimesControlScalingSEs = SetScalingSEs(exp_index,:);
    CompiledEmbryos.MasterSetWindowedDubuisTimesControlScalingIntercepts = SetScalingIntercepts(exp_index,:);
    CompiledEmbryos.MasterSetWindowedDubuisTimesControlScalingInterceptSEs = SetScalingInterceptSEs(exp_index,:);
    AllCompiledEmbryos{exp_index} = CompiledEmbryos;
...
for i = 1:NumSets
    counts_windowed(:,:,:,i) =repmat( (AllCompiledEmbryos{1}.DubuisTimesSmoothedProfiles.ControlCounts.counts_within_1sigma+...
        AllCompiledEmbryos{1}.DubuisTimesSmoothedProfiles.ControlCounts.counts_within_2sigma).',1, NumAPbins,NChannels);
...
for exp_index = 1:length(AllSetInfo.Temperatures)
    ...
    CEoutpath = [OutEmbryoPath, filesep, 'CompiledEmbryos.mat'];
    CompiledEmbryos = AllCompiledEmbryos{exp_index};
    CompiledEmbryos.MasterSetSmoothedDubuisTimesControlScalingFactors = SetScalingFactors(exp_index,:);
    CompiledEmbryos.MasterSetSmoothedDubuisTimesControlScalingSEs = SetScalingSEs(exp_index,:);
    CompiledEmbryos.MasterSetSmoothedDubuisTimesControlScalingIntercepts = SetScalingIntercepts(exp_index,:);
    CompiledEmbryos.MasterSetSmoothedDubuisTimesControlScalingInterceptSEs = SetScalingInterceptSEs(exp_index,:);
    AllCompiledEmbryos{exp_index} = CompiledEmbryos;
    save(CEoutpath, 'CompiledEmbryos');
end

%% Fit Universal Scaling using DeltaFC Embryo Times
exp_idx = 1:NumSets;
AllSets = 1:NumSets;
...
counts_3sigma = zeros(NumSets, size(AllCompiledEmbryos{1}.FixCorrectedSmoothedProfiles.ControlCounts.counts_within_2sigma, 2));
...
for i = 1:NumSets
    ...
    counts_3sigma(i,:) = AllCompiledEmbryos{exp_idx(i)}.FixCorrectedSmoothedProfiles.ControlCounts.counts_within_3sigma;
