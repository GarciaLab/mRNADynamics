function TemperatureComparisonPlots(AllCompiledEmbryos)
%%
APbins = 0:0.025:1;
        ...
        for time_index = 1:length(times_to_plot)
            ...
            for temp_index = 1:length(temps)
                ...
                for rep_index = 1:2
                    ...
                    timebins = find((x_sample{rep_index, temp_index} <= times_to_plot(time_index) + 2.5)& ...
                        (x_sample{rep_index, temp_index} >= times_to_plot(time_index)-2.5));
                    if ~isempty(timebins)
...
for master_index = [1 2 7]
    for ch_index = [3 5]
        ...
        outpath2 = [AllSetsProfFigPath, filesep, 'TemperatureComparisonAPProfiles', filesep, 'BgdSubNormalizedSingleEmbryoTestProfiles', filesep,...
            ChannelNames{ch_index}, '_MasterSet',num2str(master_index),'_NC13MeanProfile.png' ];
        saveas(DeltaFCFixCorrectedFig,outpath2);
...
for master_index = [1 2 7]
    for ch_index = [3 5]
        ...
        outpath2 = [AllSetsProfFigPath, filesep, 'TemperatureComparisonAPProfiles', filesep, 'BgdSubNormalizedSingleEmbryoTestProfiles', filesep,...
            ChannelNames{ch_index}, '_MasterSet',num2str(master_index),'_NC13MeanP90Profile.png' ];
        saveas(DeltaFCFixCorrectedFig,outpath2);
...
for master_index = [1 2 7]
    for ch_index = [3 5]
        ...
        outpath2 = [AllSetsProfFigPath, filesep, 'TemperatureComparisonAPProfiles', filesep, 'BgdSubNormalizedSingleEmbryoTestProfiles', filesep,...
            ChannelNames{ch_index}, '_MasterSet',num2str(master_index),'_NC13MeanP75Profile.png' ];
        saveas(DeltaFCFixCorrectedFig,outpath2);
...
for master_index = [1 2 7]
    for ch_index = [3 5]
        ...
        outpath2 = [AllSetsProfFigPath, filesep, 'TemperatureComparisonAPProfiles', filesep, 'BgdSubNormalizedSingleEmbryoTestProfiles', filesep,...
            ChannelNames{ch_index}, '_MasterSet',num2str(master_index),'_NC13MeanP25toP75Profile.png' ];
        saveas(DeltaFCFixCorrectedFig,outpath2);
...
NChannels = 5;
NMasterProfs = 8;
fieldnames1 = fieldnames(AllCompiledEmbryos{1}.NC13);
fieldnames2 =  fieldnames(AllCompiledEmbryos{1}.NC13.BgdSubNormalizedUnivScaledProfiles);
for exp_index = 1:NumSets
     CompiledEmbryos = AllCompiledEmbryos{exp_index};
    for f_index = 1:length(fieldnames1)
        for f2_index = 1:length(fieldnames2)
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfile = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSD = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfileP50 = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSDP50 = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfileP75 = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSDP75 = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfileP90 = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSDP90 = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfileP25toP75 = cell(NChannels, NumMasterProfs);
            CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSDP25toP75 = cell(NChannels, NumMasterProfs);
            for ch_index = [3 5]
                for master_index = 1:8
                   
                    ys =  CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.Test(:,:,ch_index, master_index);
                    BadTFs=  sum(isnan(ys), 2).' >=NumAPbins;
                    ys= ys(~BadTFs,:);
                    y_diffs = (max(ys , [], 2)-min(ys, [], 2)).';
                    P = prctile(y_diffs, [25 50 75 90]);
                    mean_y_25p = mean(ys(y_diffs > P(1),:), 1, 'omitnan');
                    sd_y_25p = std(ys(y_diffs > P(1),:), 1, 'omitnan');
                    mean_y_50p= mean(ys(y_diffs > P(2),:), 1, 'omitnan');
                    sd_y_50p = std(ys(y_diffs > P(2),:), 1, 'omitnan');
                    mean_y_75p= mean(ys(y_diffs > P(3),:), 1, 'omitnan');
                    sd_y_75p = std(ys(y_diffs > P(3),:), 1, 'omitnan');
                    mean_y_90p= mean(ys(y_diffs > P(4),:), 1, 'omitnan');
                    sd_y_90p = std(ys(y_diffs > P(4),:), 1, 'omitnan');
                    mean_y_all = mean(ys, 1, 'omitnan');
                    sd_y_all= std(ys, 1, 'omitnan');
                    mean_y_25pto75p = mean(ys(y_diffs > P(2) & y_diffs <= P(3)  ,:), 1, 'omitnan');
                    sd_y_25pto75p= std(ys(y_diffs > P(2)& y_diffs <= P(3),:), 1, 'omitnan');
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfile{ch_index, master_index} = mean_y_all;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSD{ch_index, master_index} = sd_y_all;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfileP50{ch_index, master_index} = mean_y_50p;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSDP50{ch_index, master_index} = sd_y_50p;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfileP75{ch_index, master_index} = mean_y_75p;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSDP75{ch_index, master_index} = sd_y_75p;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfileP90{ch_index, master_index} = mean_y_90p;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSDP90{ch_index, master_index} = sd_y_90p;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.MeanProfileP25toP75{ch_index, master_index} = mean_y_25pto75p;
                    CompiledEmbryos.NC13.(fieldnames1{f_index}).(fieldnames2{f2_index}).ControlScaling.ProfileSDP25toP75{ch_index, master_index} = sd_y_25pto75p;
...
save([AllCompiledPath, 'AllCompiledEmbryos.mat'], 'AllCompiledEmbryos', '-v7.3');%,
