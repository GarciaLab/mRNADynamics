...
function outmat = getMarkAndFindMovieMat(liveExperiment)
...
NReplicates = max(MarkAndFindInfo.NReplicates);

if haveTifStacks
    
    
    moviePrecision = 'uint16';
    outmat = zeros(liveExperiment.yDim, liveExperiment.xDim,...
        liveExperiment.zDim, MarkAndFindInfo.NSeries,NReplicates, ...
    ...
    for ch = channelsToRead
        ...
        for rep = 1:NReplicates
            ...
            preChDir = preTifDir( ...
                contains(...
                string({preTifDir.name}), [ iIndex(rep,3),'_ch0', num2str(ch)]) &...
