...
[Prefix, ForceAP, SkipTraces, SkipFluctuations, SkipFits, SkipMovie, ...
    ...
    optionalResults, yToManualAlignmentPrompt, minBinSize, edgeWidth] = determineCompileParticlesOptions({Prefix});
...
ChN = liveExperiment.spotChannels(1);
%%
...
if isfile(CompiledParticlesFile)
    try
        save(CompiledParticlesFile, savedVariables{:},'-append');
    catch
        save(CompiledParticlesFile, savedVariables{:},'-append', '-nocompression');
    end
else
    try
        save(CompiledParticlesFile, savedVariables{:});
    catch
        save(CompiledParticlesFile, savedVariables{:}, '-nocompression');
    end
end
