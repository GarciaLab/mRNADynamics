function PlotLTMBinnedTrapParamsVsTemp(this, parameter, outdir, varargin)
%%
...
for SetIndex =1:NumTemperatures
    ...
    eb{SetIndex} = errorbar(APbins, ones(1, length(APbins)), .1*ones(1, length(APbins)),...
        .1*ones(1, length(APbins)), .1*ones(1, length(APbins)), .1*ones(1, length(APbins)),...
        'LineStyle', 'none');
    hold on
...
for NCIndex=1:length(this.IncludedNCs)
    ...
    AllNCTemps = NaN(NumTemperatures, NumAPbins);
    AllNCTempSEs = NaN(NumTemperatures, NumAPbins);
    ...
    for SetIndex=1:NumTemperatures
        ...
        if ~isempty(IncludedBins)
            ...
            AllNCTemps(SetIndex,:) = SetTemperatures;
            AllNCTempSEs(SetIndex,:) = SetSETemperatures;
    ...
    for APindex = 1:NumAPbins
        ...
        for SetIndex=1:NumTemperatures
            if ~isnan(AllNCParamSEs(SetIndex, APindex))
                NoDataCondition = isnan(AllNCParams(SetIndex, APindex)) | (AllNCParams(SetIndex, APindex)./AllNCParamSEs(SetIndex, APindex) <  1) | ...
                    (AllCounts(SetIndex,APindex) <  this.MinimumBinCount);
            else
                NoDataCondition = isnan(AllNCParams(SetIndex, APindex)) | (AllCounts(SetIndex,APindex) <  this.MinimumBinCount);
            end
            ...
            if  NoDataCondition
                ...
                if IncludeFits
                    ...
                    FrameProfAx2.Children(end-(2*(SetIndex-1))-2).XPositiveDelta = 1;
                    FrameProfAx2.Children(end-(2*(SetIndex-1))-2).XNegativeDelta = 1;
                ...
                FrameProfAx.Children(end-(2*(SetIndex-1)+1)).XData =AllNCTemps(SetIndex, APindex);
                FrameProfAx.Children(end-(2*(SetIndex-1))).YData =  AllNCParams(SetIndex, APindex);
                FrameProfAx.Children(end-(2*(SetIndex-1))).XData = AllNCTemps(SetIndex, APindex);
                ...
                FrameProfAx.Children(end-(2*(SetIndex-1))).XPositiveDelta = AllNCTempSEs(SetIndex,APindex);
                FrameProfAx.Children(end-(2*(SetIndex-1))).XNegativeDelta  = AllNCTempSEs(SetIndex,APindex);
                ...
                if IncludeFits
                    FrameProfAx2.Children(end-(2*(SetIndex-1)+1)-2).XData =1/(R*(AllNCTemps(SetIndex, APindex)+273));
                    FrameProfAx2.Children(end-(2*(SetIndex-1)+1)-2).YData = AllNCParams(SetIndex, APindex);
                    FrameProfAx2.Children(end-(2*(SetIndex-1))-2).XData = 1/(R*(AllNCTemps(SetIndex, APindex)+273));
                    ...
                    se = sqrt((R/(R*(AllNCTemps(SetIndex, APindex)+273)^2))^2*AllNCTempSEs(SetIndex,APindex)^2);
                    FrameProfAx2.Children(end-(2*(SetIndex-1))-2).XPositiveDelta = se;
                    FrameProfAx2.Children(end-(2*(SetIndex-1))-2).XNegativeDelta = se;
                    ...
                    FrameProfAx2.Children(end-(2*(SetIndex-1)+1)).XData =1/(R*(AllNCTemps(SetIndex, APindex)+273));
                    FrameProfAx2.Children(end-(2*(SetIndex-1))).YData =  AllNCParams(SetIndex, APindex);
                    FrameProfAx2.Children(end-(2*(SetIndex-1))).XData = 1/(R*(AllNCTemps(SetIndex, APindex)+273));
                    ...
                    se = sqrt((R/(R*(AllNCTemps(SetIndex, APindex)+273)^2))^2*AllNCTempSEs(SetIndex,APindex)^2);
                    FrameProfAx2.Children(end-(2*(SetIndex-1))).XPositiveDelta = se;
                    FrameProfAx2.Children(end-(2*(SetIndex-1))).XNegativeDelta  = se;
        ...
        for SetIndex = 1:NumTemperatures
            ...
            legend_labels3{1, length(legend_labels3)+1} = [num2str(temperatures(SetIndex)), ' °C'];
        ...
        outpath = [outdir4, filesep,'Binned', OutputString,  '_NC',num2str(NC), '_AP', num2str(APindex), '.png'];
