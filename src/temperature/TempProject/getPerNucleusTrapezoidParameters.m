function params = getPerNucleusTrapezoidParameters(this, ParamType, TraceType, getSE)
if ~exist('getSE', 'var')
    getSE = false;
end
...
elseif strcmp(lower(TraceType), 'fluo') | strcmpi(lower(TraceType), 'unaligned')
    if ~getSE
        traceName = 'Unaligned';
    else
        traceName = 'UnalignedStdError';
    end
elseif strcmp(lower(TraceType), 'fluo3d')| strcmpi(lower(TraceType), 'unaligned3d')
    if ~getSE
...
params = this.PerNucleusProfileParameters.(paramName).(traceName);
