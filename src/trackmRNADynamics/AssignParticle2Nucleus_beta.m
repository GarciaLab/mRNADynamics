...
if ~isempty(NewSpotsX)
    ...
    [MinValues,MinIndex]=min(Distance,[],2);    
               
    %If retrack, set the distance of the already assigned spots to
    %infinity
    %if retrack % NL: this almost certainly does nothing
    ...
    AssignedSchnitz=zeros(1,length(Particles));
    for i=1:length(Particles)
    ...
    %Find the schnitz that was closest to each of the spots found in this frame.
    ...
    for i=1:length(UniqueMinIndexSchnitz)
       ...
       %Find the particles that are assigned to this schnitz
        ...
        if ~isempty(ParticleToAssign)
            ...
            %have to see whether this leads to any issues.
            PreviousParticlesX=[];
            PreviousParticlesY=[];
