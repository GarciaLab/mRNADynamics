...
%     ParticlesTemp.logLMean = nanmean(ParticlesTemp.logL);
    ...
    ParticlesTemp.xPosSEInf = KFTrack1.smoothedTrackSE(:,1);
    ParticlesTemp.yPosSEInf = KFTrack1.smoothedTrackSE(:,2);
    ParticlesTemp.zPosSEInf = KFTrack1.smoothedTrackSE(:,3);
    ...
    ParticlesTemp.xPosSEInf(addIndicesTo) = KFTrack2.smoothedTrackSE(addIndicesFrom,1);
    ParticlesTemp.yPosSEInf(addIndicesTo) = KFTrack2.smoothedTrackSE(addIndicesFrom,2);
    ParticlesTemp.zPosSEInf(addIndicesTo) = KFTrack2.smoothedTrackSE(addIndicesFrom,3);
    
    % add full prediction arrays as well
    ParticlesTemp.smoothedPredictions = KFTrack1.smoothedTrack;
    ParticlesTemp.smoothedPredictionsSE = KFTrack1.smoothedTrackSE;
    
    ParticlesTemp.smoothedPredictions(addIndicesTo,:) = KFTrack2.smoothedTrack(addIndicesFrom,:);
    ParticlesTemp.smoothedPredictionsSE(addIndicesTo,:) = KFTrack2.smoothedTrackSE(addIndicesFrom,:);
