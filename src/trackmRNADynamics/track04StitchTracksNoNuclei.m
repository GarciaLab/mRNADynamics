function StitchedParticles = track04StitchTracksNoNuclei(...
  ...
  matchCostMax = 3*.5; % maximum number of sigmas away
  matchCostMin = 0.5*.5; % cost for initial pair matching (pairs must be within 1/2 sigma to be matched)
...
%     sigmaArray(sigmaArray==0) = sqrt(realmin) * 1e8; 
    ...
    for c = 1:length(costVec)
      ...
      [matchArray,~,~] = matchpairs(mDistanceMat,costVec(c));
